    ...
    for (QVariantMap::const_iterator p = map.begin(); p != map.end(); p++)
    {
        ...
        else if (param == "xy")
        {
            ...
            if (map[param].type() == QVariant::List)
            {
                ...
                {
                    hasErrors = true;
                    rsp.list.append(errorToMap(ERR_INVALID_VALUE, QString("/groups/%1/scenes/%2").arg(QString(scene->groupAddress)).arg(QString(scene->id)), QString("parameter, xy has more than 9 items")));
                    continue;
                }
                ...
                for (auto &color : colors)
                {
                    ...
                    if (xy.length() != 2)
                    {
                        rsp.list.append(errorToMap(ERR_INVALID_VALUE, QString("/groups/%1/scenes/%2").arg(QString(scene->groupAddress)).arg(QString(scene->id)), QString("invalid length, %1, for parameter, xy/%2").arg(xy.length()).arg(i)));
                        ok = false;
                        continue;
                    }
                    double x = xy[0].toDouble(&check);
                    if (!check || x < 0 || x > 1)
                    {
                        rsp.list.append(errorToMap(ERR_INVALID_VALUE, QString("/groups/%1/scenes/%2").arg(QString(scene->groupAddress)).arg(QString(scene->id)), QString("invalid value, %1, for parameter, xy/%2/0").arg(xy[0].toString()).arg(i)));
                        ok = false;
                    }
                    if (x > maxX)
                    ...
                    if (!check || y < 0 || y > 1)
                    {
                        rsp.list.append(errorToMap(ERR_INVALID_VALUE, QString("/groups/%1/scenes/%2").arg(QString(scene->groupAddress)).arg(QString(scene->id)), QString("invalid value, %1, for parameter, xy/%2/1").arg(xy[1].toString()).arg(i)));
                        ok = false;
                    }
                    if (y > maxY)
